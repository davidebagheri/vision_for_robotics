...
cv::Matx34f estimatePoseDLT(const std::vector<cv::Point2f>& point_2d, 
    ...
    cv::Mat M_vec = svd.vt.row(point_2d.size()*2 - 1);
    cv::Mat M = cv::Mat(3, 4, CV_32F, M_vec.data);

    // Enforce R in M = [R|t] to have determinant = 1
    if (M.at<float>(3, 4) > 0) M = M * -1;

    // Solve Orthogonal Procrustes Problem to make R be SO(3) 
    cv::Mat R = M(cv::Range(0,3), cv::Range(0,3));
    cv::SVD svd_R = cv::SVD(R);
    cv::Mat R_hat = svd_R.u * svd_R.vt;

    // Recover scale
    float scale = cv::norm(R_hat) / cv::norm(R);
    cv::Mat t_hat = scale * M(cv::Range(0,3), cv::Range(3,4));
    
    // Copy to final result
    cv::Matx34f pose;

    // Rotation
    pose(0,0) = R_hat.at<float>(0, 0);
    pose(0, 1) = R_hat.at<float>(0, 1);
    pose(0, 2) = R_hat.at<float>(0, 2);
    pose(1, 0) = R_hat.at<float>(1, 0);
    pose(1, 1) = R_hat.at<float>(1, 1);
    pose(1, 2) = R_hat.at<float>(1, 2);
    pose(2, 0) = R_hat.at<float>(2, 0);
    pose(2, 1) = R_hat.at<float>(2, 1);
    pose(2, 2) = R_hat.at<float>(2, 2);
    // Translation
    pose(0, 3) = t_hat.at<float>(0, 0);
    pose(1, 3) = t_hat.at<float>(0, 1);
    pose(2, 3) = t_hat.at<float>(0, 2);
...
cv::Mat getLinearSysMat(const std::vector<cv::Point2f>& points_2d,
                        ...
                        const cv::Matx33f& camera_matrix_inv)
{
    cv::Mat Q = cv::Mat::zeros(cv::Size(12, points_2d.size() * 2), CV_32F);
    ...
    for (int i = 0; i < points_2d.size(); i++){
        ...
        Q.at<cv::Vec4f>(mat_row, 2) = h_point3d * (-point_2d.x);
        Q.at<cv::Vec4f>(mat_row+1, 1) = h_point3d;
        Q.at<cv::Vec4f>(mat_row+1, 2) = h_point3d * (-point_2d.y);
    }
